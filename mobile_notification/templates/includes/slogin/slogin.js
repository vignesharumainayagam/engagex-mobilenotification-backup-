...
login.bind_events = function() {
  ...
  $(".form-signup").on("submit", function(event) {
    event.preventDefault();
    var args = {};
    args.cmd = "frappe.core.doctype.user.user.student_sign_up";
    args.email = ($("#signup_email").val() || "").trim();
    args.redirect_to = get_url_arg("redirect-to") || '';
    args.full_name = ($("#signup_fullname").val() || "").trim();
    args.mobile_no = ($("#signup_mobile_number").val() || "").trim();
    args.new_password = ($("#signup_password").val() || "").trim();
  ...
  var login_handlers = {
    200: function(data) {
      if(data.message == 'Logged In'){
        ...
        var roles = frappe.user_role;
        if (jQuery.inArray("Student", roles)>0) 
          {
        ...
        else if (jQuery.inArray("Student", roles)<0) 
          {
            window.location.href = "test";
            $('[data-label="test"]').hide(); 
          }  
